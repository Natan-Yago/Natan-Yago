 ...
 */
export default function ActionButton({ href, text, hoverText = text, icon, iconPosition = 'right', className = "", ...rest }) {
  ...
  useEffect(() => {
    ...
    tl.to(defaultText, { y: '-100%', opacity: 0, duration: 0.3, ease: "power2.inOut" }, 0)
      .to(hoverText, { y: '0%', opacity: 1, duration: 0.3, ease: "power2.inOut" }, 0);
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden transition-none" ref={textContainerRef}>
        <div ref={defaultTextRef} className="whitespace-nowrap block">
          {text}
        </div>
        <div className="absolute top-0 left-0 whitespace-nowrap block opacity-0 translate-y-full" ref={hoverTextRef}>
          {hoverText}
        </div>
      </div>
